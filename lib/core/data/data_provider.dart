  ...
  Future<List<Category>> getAllCategory({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredCategories =
            List.from(_allCategories); // initilized filter list with all data
        notifyListeners();
      ...
      if (response.isOk) {
        ApiResponse<List<SubCategory>> apiResponse =
            ApiResponse<List<SubCategory>>.fromJson(
                response.body,
                (json) => (json as List)
                    .map((items) => SubCategory.fromJson(items))
                    .toList());
        ...
        String errorMsg =
            response.body?['message'] ?? "Server error: ${response.statusCode}";
        if (showSnack) SnackBarHelper.showErrorSnackBar(errorMsg);
      }
    } catch (e) {
      print("Error fetching sub categories $e");
      if (showSnack) SnackBarHelper.showErrorSnackBar("Error: ${e.toString()}");
      // rethrow;
    }
...
// filterSubCategories
  void filterSubCategories(String keyword) {
    if (keyword.isEmpty) {
      ...
      final lowerKeword = keyword.toLowerCase();
      _filteredSubCategories = _allSubCategories.where((subCategories) {
        return (subCategories.name ?? '').toLowerCase().contains(lowerKeword);
      }).toList();
    }
    notifyListeners();
  }
...
//   }
  void filterProductsByQuantity(String productQntType) {
    if (productQntType == 'All Product') {
      ...
      // Corrected: Now includes everything from 1 to 5
      _filteredProducts = _allProducts.where((product) {
        int qty = product.quantity ?? 0;
        return qty > 0 && qty <= 5;
      }).toList();
    } else if (productQntType == 'Other Stock') {
      // Corrected: Now strictly items with MORE than 5
      _filteredProducts = _allProducts.where((product) {
        return (product.quantity ?? 0) > 5;
      }).toList();
    } else {
      _filteredProducts = List.from(_allProducts);
    }
...
//   }
  int calculateProductWithQuantity({int? quantity}) {
    ...
    if (quantity == null) {
      ...
      // Count Out of Stock (exactly 0)
      count = _allProducts.where((p) => p.quantity == 0).length;
    } else if (quantity == 5) {
      // Count Limited Stock (1 to 5)
      count = _allProducts
          .where(
              (p) => p.quantity != null && p.quantity! > 0 && p.quantity! <= 5)
          .length;
    } else {
      // Count Other Stock (greater than 5)
      count = _allProducts
