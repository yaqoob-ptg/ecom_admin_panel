...
class BrandHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Mobile S (<480px): title on top, search below
    if (isMobileS) {
      return Column(
        ...
        children: [
          ...
          BrandSearchField(
            onChange: (val) => context.dataProvider.filterBrands(val),
          ),
        ],
      );
    }
...
class BrandSearchField extends StatelessWidget {
  final Function(String) onChange;
  const BrandSearchField({Key? key, required this.onChange}) : super(key: key);
