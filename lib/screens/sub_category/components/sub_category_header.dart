...
class SubCategoryHeader extends StatelessWidget {
  /// Set to false when ProfileCard is already rendered by the parent scaffold.
  final bool showProfile;
  const SubCategoryHeader({Key? key, this.showProfile = true})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    // Mobile S (<480px): title on top row (+ profile if shown), search below
    if (isMobileS) {
      return Column(
        ...
        children: [
          ...
          _SubCategorySearchField(
            onChange: (val) => context.dataProvider.filterSubCategories(val),
          ),
        ],
      );
    }
...
class _SubCategorySearchField extends StatelessWidget {
  final Function(String) onChange;
  const _SubCategorySearchField({required this.onChange});
